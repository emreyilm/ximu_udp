...
namespace x_IMU_GUI
{
    public partial class Form_main : Form
    {
        ...
        void xIMUserial_xIMUdataReceived(object sender, x_IMU_API.xIMUdata e)
        {
            if (dataLoggerCSVfileWriter != null)
            {
                try
                {
                    ...
                    //MessageBox.Show(textBox_UDPdataSend.Text, "UDP port is opened", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //while (true) {

                    //UDPSend(num, "127.0.0.1", 7001); num = num + 1;
        ...
        */
        ...
        /// ToggleButton Click event toggles data logging start/stop.
        /// </summary>
        private void toggleButton_UDPdataSendStartStopSending_Click(object sender, EventArgs e)
        {
            if ((sender as ToggleButton).ToggleState)
            {
                try
                {
                    /*dataLoggerCSVfileWriter = new x_IMU_API.CSVfileWriter(textBox_dataLoggerFilePath.Text);
                    textBox_UDPdataSendAddress.Enabled = false;*/
                    int milliseconds = 200;
                    MessageBox.Show(textBox_UDPdataSend.Text, "UDP port is opened", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //while (true) {

                        //UDPSend(num, "127.0.0.1", 6001); num = num + 1; 
                        System.Threading.Thread.Sleep(milliseconds);
                    //}
            ...
            else
            {
                ...
                CSVfileWriterResults.Show(dataLoggerCSVfileWriter, "Data Logger Results");
                dataLoggerCSVfileWriter = null;
                textBox_UDPdataSend.Enabled = true;*/
            }
